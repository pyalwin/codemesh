...
import { detectLanguageServer, getLspClient, shutdownAllLspClients, __test_reapIdle, __test_stopHeartbeat, } from "./lsp-client.js";
...
describe("LSP idle reaping", () => {
    ...
    afterEach(async () => {
        if (origIdleMs === undefined)
            delete process.env.CODEMESH_LSP_IDLE_MS;
        else
            process.env.CODEMESH_LSP_IDLE_MS = origIdleMs;
    ...
    it("removes the client from the cache after idle threshold", async () => {
        ...
        if (!client)
            return; // no LSP installed on this machine — skip
...
//# sourceMappingURL=lsp-client.test.js.map