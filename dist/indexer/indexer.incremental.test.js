...
describe("Indexer — incremental embeddings", () => {
    ...
    it("does not re-embed symbols in unchanged files on incremental index", async () => {
        writeFileSync(join(projectRoot, "a.ts"), "export function alpha() { return 1; }\n");
        writeFileSync(join(projectRoot, "b.ts"), "export function beta() { return 2; }\n");
        ...
        // Touch only b.ts
        writeFileSync(join(projectRoot, "b.ts"), "export function beta() { return 99; }\n");
        ...
        const ldb = await lancedb.connect(join(projectRoot, ".codemesh", "vectors"));
...
describe("Indexer — transaction chunking", () => {
    ...
    it("indexes successfully across multiple file batches", async () => {
        ...
        // untested.
        for (let i = 0; i < 600; i++) {
            writeFileSync(join(projectRoot, `mod${i}.ts`), `export const value${i} = ${i};\n`);
        }
        const indexer = new Indexer(backend, projectRoot);
...
//# sourceMappingURL=indexer.incremental.test.js.map