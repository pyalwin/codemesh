...
function loadQuery(language, languageName, queryFile) {
    ...
    const query = new Parser.Query(language, src);
...
export function parseWithQuery(tree, language, languageName, queryFile, source) {
    ...
    for (const m of matches) {
        ...
        for (const [captureName, node] of byName) {
            ...
            symbols.push({
                ...
                lineEnd,
                signature: source
                    .slice(nodeCapture.startIndex, Math.min(nodeCapture.endIndex, nodeCapture.startIndex + 200))
                    .split("\n")[0]
                    .trim(),
                scopePath,
            });
