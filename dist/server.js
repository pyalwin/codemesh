...
export function createServer(storage, projectRoot) {
    ...
    server.tool("codemesh_explore", "Omni-tool for exploring the codebase. Use 'search' to find things by text, 'context' to see relations of a specific file/symbol, and 'impact' to find reverse dependencies (trace).", {
        ...
        if (args.action === "search") {
            ...
            if (!args.path)
                throw new Error("path or paths is required for context action");
            ...
            if (!args.path)
                throw new Error("path is required for impact action");
    ...
    // ── codemesh_enrich ─────────────────────────────────────────────
    server.tool("codemesh_enrich", "Add an AI-generated concept (summary/insight) linked to a file or symbol. Creates a ConceptNode with a describes edge.", {
    ...
    // ── codemesh_workflow ───────────────────────────────────────────
    server.tool("codemesh_workflow", "Create a workflow that links files in a logical sequence (e.g. request flow, data pipeline). Creates a WorkflowNode with traverses edges.", {
    ...
    // ── codemesh_status ─────────────────────────────────────────────
    server.tool("codemesh_status", "Get statistics about the knowledge graph: node counts by type, edge counts by type, stale count, and last indexed timestamp.", async () => {
        const result = await handleStatus(storage);
        return textResult(result, projectRoot);
    });
    // ── codemesh_trace ──────────────────────────────────────────────
    server.tool("codemesh_trace", "Trace a call chain from a symbol to its leaf nodes. Follows calls/callers edges through the graph and returns every function in the path. Use this AFTER codemesh_explore to follow a specific execution flow to completion.", {
    ...
    // ── codemesh_map ────────────────────────────────────────────────
    server.tool("codemesh_map", "Map the call graph from a query or symbol. Returns a tree of symbols with summaries — no source code. Use codemesh_source to read the code of specific symbols you need.", {
