 ...
 */
function loadGrammar(grammarPackage: string, languageName: string): unknown {
...
const EMPTY_RESULT: ParseResult = { symbols: [], imports: [], calls: [] };
...
function extractTSFunctionSignature(node: SyntaxNode): string {
  const nameNode = node.childForFieldName("name");
...
function extractTSMethodSignature(
  node: SyntaxNode,
  enclosingClass: string | null,
): string {
  const nameNode = node.childForFieldName("name");
...
function extractPythonCalleeName(node: SyntaxNode): string | null {
  if (node.type === "identifier") {
    return node.text;
  }
...
function walkGenericNode(
  ...
  if (
    node.type.includes("function_declaration") ||
  ...
  if (
    node.type.includes("class_declaration") ||
